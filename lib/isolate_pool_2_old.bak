...
class IsolatePool {
  ...
  /// should the action fail in the executing isolate
  ///
  /// - `job` - A job to be scheduled
  /// - `isolateIndex` - An index of the isolate to run the job on, if -1 the job will be scheduled on the first available isolate
  Future<T> scheduleJob<T>(PooledJob<T> job, [int isolateIndex = -1]) {
    ...
    _jobs[lastJobStartedIndex] = _PooledJobInternal<T>(job, lastJobStartedIndex, isolateIndex);
  ...
  void destroyInstance(PooledInstanceProxy instance) {
    ...
    sendPorts[index].send(_DestroyRequest(instance._instanceId));
  ...
  void _runJobWithVacantIsolate() {
    ...
    if (availableIsolate > -1 && _jobs.entries.where((j) => j.value.started == false).isNotEmpty) {
      ...
      job.started = true;
      sendPorts[availableIsolate].send(job);
  ...
  /// - [debugLabel] - A function that returns a string label for each isolate, useful for debugging. It is recommended to use the [int] parameter.
  ...
  Future start({
    ...
    for (var i = 0; i < numberOfIsolates; i++) {
      ...
      final params = _PooledIsolateParams(
        rp.sendPort,
        errorSendPort,
        i,
        sw,
  ...
  void _processJobResult(_PooledJobResult result) {
    _isolateBusyWithJob[result.isolateIndex] = false; // unfreeze the isolate
  ...
  Future _processRequest(_Request request) async {
    ...
    final sendPort = sendPorts[i.isolateIndex];
  ...
  void stop() {
    for (var i in _isolates.values) {
      ...
      for (var rPort in _poolReceivePorts.values) {
        rPort.close(); // Issue #3
      }
    }
      ...
      try {
        await runZonedGuarded(
          () async {
            ...
            params.sendPort.send(_PooledJobResult(result, message.jobIndex, message.isolateIndex, null, null));
          },
          (e, st) {
            final r = _PooledJobResult(null, message.jobIndex, message.isolateIndex, e, st);
            params.sendPort.send(r);
            params.errorSendPort.send(e);
          },
          zoneSpecification: ZoneSpecification(
            handleUncaughtError: (self, parent, zone, error, stackTrace) {
              params.sendPort.send(_PooledJobResult(null, message.jobIndex, message.isolateIndex, error, stackTrace));
              params.errorSendPort.send(error);
            },
          ),
        );
      } catch (e, st) {
        params.sendPort.send(_PooledJobResult(null, message.jobIndex, message.isolateIndex, e, st));
        params.errorSendPort.send(e);
      }
